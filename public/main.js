...
const services = headerWrapper.querySelector('.services');
const {height} = headerWrapper.getBoundingClientRect();
const serviceTop = height - services.getBoundingClientRect().height;
...
function setPosition(perc){
  ...
  headerBg.style.setProperty("--yOffset", `${20 * (perc * height) / window.innerHeight}%`);
  

  // const pos = perc * height;
  // if (pos >= serviceTop && pos <= height) {
  //   const boy    = document.querySelector('.services-bg_boy');
  //   const mother = document.querySelector('.services-bg_mother');

  //   boy.style.setProperty('--transform', `translate(${(height - perc) / (height - serviceTop)})`)
  //   mother.style.setProperty('--transform', `translate(${(height - perc) / (height - serviceTop)}`)
  // }
}
...
//   let xOffset = x / window.innerWidth * 10;
//   let yOffset = y / window.innerHeight * 20;

// }

// document.addEventListener("mousemove", e => {
//   setParallax(e.screenX, e.screenY);
// })